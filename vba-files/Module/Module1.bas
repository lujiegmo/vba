...
Public Function 期失理由取得(toolSheet As Worksheet) As String
    ...
    ' E列25行目の値を取得
    cellValue = toolSheet.Cells(期失理由行, 期失理由列).Value

    ' ローン口座ステータスを取得
    ローン口座ステータス = ローン口座ステータス取得(toolSheet)
...
Public Function ローン口座ステータス取得(toolSheet As Worksheet) As String
    Dim cellValue As Variant

    ' C列22行目の値を取得
    cellValue = toolSheet.Cells(ローン口座ステータス行, ローン口座ステータス列).Value
...
Public Function 手続開始日取得(toolSheet As Worksheet) As Date
    Dim cellValue As Variant

    ' C列11行目の値を取得
    cellValue = toolSheet.Cells(手続開始日行, 手続開始日列).Value
...
Public Function 手続理由取得(toolSheet As Worksheet) As String
    Dim cellValue As Variant

    ' C列10行目の値を取得
    cellValue = toolSheet.Cells(手続理由行, 手続理由列).Value
...
Public Function 顧客番号取得(toolSheet As Worksheet) As String
    Dim cellValue As Variant

    ' C列6行目の値を取得
    cellValue = toolSheet.Cells(顧客番号行, 顧客番号列).Value
    ...
    ' 文字列として変換し、9桁にフォーマット
    顧客番号取得 = Format(CStr(cellValue), "000000000")
End Function

' 計算書の作成パス取得関数
' C列7行目から計算書の作成パスを取得し、空白の場合はエラーを発生させる
' パスが存在するフォルダでない場合もエラーを発生させる
Public Function 計算書の作成パス取得(toolSheet As Worksheet) As String
    Dim pathValue As Variant
    Dim pathString As String

    ' C列7行目の値を取得
    pathValue = toolSheet.Cells(計算書作成パス行, 計算書作成パス列).Value
...
Public Sub ファイル出力(toolSheet As Worksheet, templateSheet As Worksheet)
    ...
    ' 1. 計算書の作成パス取得
    出力フォルダパス = 計算書の作成パス取得(toolSheet)

    ' 2. 出力データ作成
    出力データ = 出力データ作成(toolSheet)
    ...
    ' 7.7.1. 手続開始日と期失日の整合性チェック
    If 手続開始日 < 期失日 Then
        Err.Raise 13, "利息計算書作成", "手続開始日（" & Format(手続開始日, "yyyy/mm/dd") & "）は期失日（" & Format(期失日, "yyyy/mm/dd") & "）以降の日付である必要があります。"
    End If

    ' 7.8. 期失理由設定
    Dim 期失理由 As String
    期失理由 = 期失理由取得(toolSheet)
    ...
    ' 8.5. 新しいワークブックの内容をテンプレート_WORDシートに値コピー
    Dim テンプレートワードシート As Worksheet
    Set テンプレートワードシート = toolSheet.Parent.Worksheets(テンプレートワードシート名)
         
    ' テンプレートワードシートの既存内容をクリア
    テンプレートワードシート.UsedRange.ClearContents

    ' 出力タイトル行から最終行までの範囲を取得
    Dim 最終行 As Long
    最終行 = 新しいワークシート.UsedRange.SpecialCells(xlCellTypeLastCell).Row
    ...
    ' 10. 完了メッセージ
    MsgBox "利息計算書ファイルの出力が完了しました。" & vbCrLf & "Excel保存先: " & 保存ファイルパス & vbCrLf & "Word保存先: " & ワードファイルパス, vbInformation, "ファイル出力完了"
...
Public Function 返済予定情報取得(toolSheet As Worksheet) As Variant
    ...
    Do While toolSheet.Cells(currentRow, 返済予定日列).Value <> ""
        rowCount = rowCount + 1
    ...
    For i = 1 To rowCount
        ...
        dateValue = toolSheet.Cells(currentRow, 返済予定日列).Value

        If dateValue = "" Or IsEmpty(dateValue) Then
            ' 空白の場合、一行目であれば返済元金列もチェック
            If i = 1 Then
                ' 一行目の場合、返済元金列も空白であれば日付初期値を設定
                Dim principalCheck As Variant
                principalCheck = toolSheet.Cells(currentRow, 返済元金列).Value
                If principalCheck = "" Or IsEmpty(principalCheck) Then
                    dataArray(i, 1) = 日付初期値
                Else
                    Err.Raise 13, "返済予定情報取得", currentRow & "行目のC列（返済予定日）が空白ですが、D列（返済元金）にデータが設定されています。両方とも空白にするか、両方ともデータを入力してください。"
                End If
            Else
                Err.Raise 13, "返済予定情報取得", currentRow & "行目のC列（返済予定日）が空白です。一行目以外は日付を入力してください。"
            End If
        Else
            ' 日付型チェック
            ...
            ' 前レコードの日付と同じか、より小さい場合、エラーとする
            If i > 1 Then
                if dataArray(i, 1) <= dataArray(i - 1, 1) Then
                    Err.Raise 13, "返済予定情報取得", "返済予定日が前のレコードより小さいか同じです。行: " & currentRow & "（前レコード: " & dataArray(i - 1, 1) & "、現在: " & dataArray(i, 1) & "）"
                End If
            End If
        End If
        ...
        principalValue = toolSheet.Cells(currentRow, 返済元金列).Value

        If principalValue = "" Or IsEmpty(principalValue) Then
            ' 空白の場合、一行目であれば返済予定日列もチェック
            If i = 1 Then
                ' 一行目の場合、返済予定日列も空白であれば0を設定
                Dim dateCheck As Variant
                dateCheck = toolSheet.Cells(currentRow, 返済予定日列).Value
                If dateCheck = "" Or IsEmpty(dateCheck) Then
                    dataArray(i, 2) = 0
                Else
                    Err.Raise 13, "返済予定情報取得", currentRow & "行目のD列（返済元金）が空白ですが、C列（返済予定日）にデータが設定されています。両方とも空白にするか、両方ともデータを入力してください。"
                End If
            Else
                Err.Raise 13, "返済予定情報取得", currentRow & "行目のD列（返済元金）が空白です。数値を入力してください。"
            End If
        Else
            ' 数値型チェック
        ...
        ' C列が空白になったら終了（35行目以外）
        If i > 1 And (toolSheet.Cells(currentRow, 返済予定日列).Value = "" Or IsEmpty(toolSheet.Cells(currentRow, 返済予定日列).Value)) Then
            Exit For
        End If
    Next i
...
Public Function 出力データ作成(toolSheet As Worksheet) As Variant
    ...
    ' 1. 返済予定情報取得
    返済予定データ = 返済予定情報取得(toolSheet)
    入出金データ = 入出金情報全体取得(toolSheet)
    借入利率データ = 借入利率取得(toolSheet)
    遅延損害金利率データ = 遅延損害金利率取得(toolSheet)
    計算期間最初日 = 計算期間最初日取得(toolSheet)
    ...
    For i = 2 To UBound(返済予定データ, 1)
        ...
        For j = 1 To レコード数
            ...
            入出金情報全体データ_借入返済 = 入出金情報全体取得(toolSheet)
            ...
            For j = 0 To 延滞分割日数
                ...
                If 延滞セグメント開始日 <= 延滞セグメント終了日 Then
                    ...
                    If j = 0 Then
                        ...
                        入出金情報全体データ_借入返済_延滞 = 入出金情報全体取得(toolSheet)
    ...
    期失日_期失レコード = 期失日取得(toolSheet)
    昨日 = Date - 1
    手続開始日_期失レコード = 手続開始日取得(toolSheet)
    入出金情報全体データ = 入出金情報全体取得(toolSheet)
    返済履歴データ = 返済履歴情報取得(toolSheet)
    ...
    For j = 1 To 期失レコード数
        ...
        ' イベント
        If 出力結果(出力行数, 出力_計算期間開始日列) = 手続開始日_期失レコード Then
            出力結果(出力行数, 出力_イベント列) = 手続理由取得(toolSheet)
        ElseIf 出力結果(出力行数, 出力_計算期間開始日列) = 期失日_期失レコード Then
            出力結果(出力行数, 出力_イベント列) = 期失理由取得(toolSheet)
        Else
...
Public Function 入出金情報全体取得(toolSheet As Worksheet) As Variant
    Dim 開始行 As Long
    Dim 現在行 As Long
    ...
    ' データ行数をカウント（B列が空白になるまで、最大20行まで）
    Do While toolSheet.Cells(現在行, 入出金日列).Value <> "" And 行数 < 入出金情報最大行数
        行数 = 行数 + 1
        現在行 = 現在行 + 1
    Loop

    ' データが存在しない場合はエラーを発生させる
    If 行数 = 0 Then
        Err.Raise 1000, "入出金情報全体取得", "入出金情報データが設定されていません。"
    End If

    ' 配列を初期化（行数 x 5列）
    ReDim データ配列(1 To 行数, 1 To 5)
    ...
    For i = 1 To 行数
        ' B列：入出金日（日付チェック）
        Dim 日付値 As Variant
        日付値 = toolSheet.Cells(現在行, 入出金日列).Value
        If Not IsDate(日付値) Then
            Err.Raise 13, "入出金情報全体取得", 現在行 & "行目のB列（入出金日）が日付ではありません。"
        End If
        データ配列(i, 1) = CDate(日付値)

        ' 前レコードの日付より小さい場合、エラーとする
        If i > 1 Then
            if データ配列(i, 1) < データ配列(i - 1, 1) Then
                Err.Raise 13, "入出金情報全体取得", "入出金日が前のレコードより小さいです。行: " & 現在行 & "（前レコード: " & データ配列(i - 1, 1) & "、現在: " & データ配列(i, 1) & "）"
            End If
        End If

        ' C列：摘要（文字列、空白チェック）
        Dim 摘要値 As Variant
        摘要値 = toolSheet.Cells(現在行, 摘要列).Value
        If 摘要値 = "" Or IsEmpty(摘要値) Then
            Err.Raise 13, "入出金情報全体取得", 現在行 & "行目のC列（摘要）が空白です。"
        End If
        データ配列(i, 2) = CStr(摘要値)

        ' D列：入出金金額（空白チェック・数値チェック）
        Dim 金額値 As Variant
        金額値 = toolSheet.Cells(現在行, 入出金金額列).Value
        If 金額値 = "" Or IsEmpty(金額値) Then
            Err.Raise 13, "入出金情報全体取得", 現在行 & "行目のD列（入出金金額）が空白です。"
        End If
        If Not IsNumeric(金額値) Then
            Err.Raise 13, "入出金情報全体取得", 現在行 & "行目のD列（入出金金額）が数値ではありません。"
        End If
        データ配列(i, 3) = CDbl(金額値)

        ' E列：残高（空白チェック・数値チェック）
        Dim 残高値 As Variant
        残高値 = toolSheet.Cells(現在行, 残高列).Value
        If 残高値 = "" Or IsEmpty(残高値) Then
            Err.Raise 13, "入出金情報全体取得", 現在行 & "行目のE列（残高）が空白です。"
        End If
        If Not IsNumeric(残高値) Then
            Err.Raise 13, "入出金情報全体取得", 現在行 & "行目のE列（残高）が数値ではありません。"
        End If
        データ配列(i, 4) = CDbl(残高値)

        ' F列：延滞中の約定返済元金合計（入力があれば数値チェック）
        Dim 元金値 As Variant
        元金値 = toolSheet.Cells(現在行, 約定返済元金列).Value
        If 元金値 <> "" Then
            If Not IsNumeric(元金値) Then
                Err.Raise 13, "入出金情報全体取得", 現在行 & "行目のF列（延滞中の約定返済元金合計）が数値ではありません。"
            End If
            データ配列(i, 5) = CDbl(元金値)
        Else
    ...
    ' 最終行以降に不正なデータがないかチェック
    Dim チェック行 As Long
    For チェック行 = 現在行 To 入出金開始行 + 入出金情報最大行数 - 1
        If toolSheet.Cells(チェック行, 摘要列).Value <> "" Or _
           toolSheet.Cells(チェック行, 入出金金額列).Value <> "" Or _
           toolSheet.Cells(チェック行, 残高列).Value <> "" Or _
           toolSheet.Cells(チェック行, 約定返済元金列).Value <> "" Then
            If チェック行 = 現在行 Then
                Err.Raise 13, "入出金情報全体取得", "入出金情報が設定されていません。"
            Else
                Err.Raise 13, "入出金情報全体取得", "入出金情報データに空白行があります。" & (チェック行 - 入出金開始行 + 1) & "行目にデータが設定されていますが、途中に空白行が存在します。"
            End If
        End If
    Next チェック行
    ...
    ' 同じ日付データの処理
    For i = 1 To 行数 - 1
        If データ配列(i, 1) <> 日付初期値 Then ' 有効なレコードのみチェック
            For j = i + 1 To 行数
                If データ配列(j, 1) <> 日付初期値 And データ配列(i, 1) = データ配列(j, 1) Then ' 同じ日付
                    Dim i返済分フラグ As Boolean
                    Dim j返済分フラグ As Boolean

                    ' 返済分かどうかを判定
                    i返済分フラグ = (Len(データ配列(i, 2)) >= Len(返済摘要返済分文字列) And Right(データ配列(i, 2), Len(返済摘要返済分文字列)) = 返済摘要返済分文字列)
                    j返済分フラグ = (Len(データ配列(j, 2)) >= Len(返済摘要返済分文字列) And Right(データ配列(j, 2), Len(返済摘要返済分文字列)) = 返済摘要返済分文字列)

                    If Not i返済分フラグ And Not j返済分フラグ Then
                        ' 両方とも返済分ではない場合はエラー
                        Err.Raise 13, "入出金情報全体取得", "同じ日付に複数の入出金データが存在します。日付：" & データ配列(i, 1) & "（まだ未対応のため）"
                    ElseIf i返済分フラグ And j返済分フラグ Then
                        ' 両方とも返済分の場合はエラー
                        Err.Raise 13, "入出金情報全体取得", "同じ日付に複数の返済分データが存在します。日付：" & データ配列(i, 1)
                    ElseIf i返済分フラグ Then
                        ' iが返済分の場合、iを無効にする
                        データ配列(i, 1) = 日付初期値
                    Else
    ...
    ' フィルタリング後の配列を作成
    Dim フィルタ後配列() As Variant
    ReDim フィルタ後配列(1 To 有効件数, 1 To 5)
    Dim 有効インデックス As Long
    有効インデックス = 0
...
Public Function 期失日取得(toolSheet As Worksheet) As Date
    Dim cellValue As Variant

    ' セル値を取得
    cellValue = toolSheet.Cells(期失日行, 期失日列).Value

    ' 空白チェック
    If cellValue = "" Or IsEmpty(cellValue) Then
        Err.Raise 13, "期失日取得", "期失日が設定されていません。期失日を入力してください。"
    End If

    ' 日付型かチェック
    If Not IsDate(cellValue) Then
        Err.Raise 13, "期失日取得", "期失日が日付型ではありません。正しい日付を入力してください。"
    End If
...
' 初回借入日取得関数
' C22セルから初回借入日を取得する
Public Function 初回借入日取得(toolSheet As Worksheet) As Date
    Dim cellValue As Variant

    ' セル値を取得
    cellValue = toolSheet.Cells(初回借入日行, 初回借入日列).Value

    ' 空白チェック
    If cellValue = "" Or IsEmpty(cellValue) Then
        Err.Raise 13, "初回借入日取得", "初回借入日が設定されていません。C22セルに初回借入日を入力してください。"
    End If

    ' 日付型チェック
    If Not IsDate(cellValue) Then
        Err.Raise 13, "初回借入日取得", "初回借入日が日付型ではありません。C22セルに正しい日付を入力してください。"
    End If

    ' 日付型に変換して返す
    初回借入日取得 = CDate(cellValue)
End Function

' 契約期限日取得関数
' C23セルから契約期限日を取得する
Public Function 契約期限日取得(toolSheet As Worksheet) As Date
    Dim cellValue As Variant

    ' セル値を取得
    cellValue = toolSheet.Cells(契約期限日行, 契約期限日列).Value

    ' 空白チェック
    If cellValue = "" Or IsEmpty(cellValue) Then
        Err.Raise 13, "契約期限日取得", "契約期限日が設定されていません。C23セルに契約期限日を入力してください。"
    End If

    ' 日付型チェック
    If Not IsDate(cellValue) Then
        Err.Raise 13, "契約期限日取得", "契約期限日が日付型ではありません。C23セルに正しい日付を入力してください。"
    End If

    ' 日付型に変換して返す
    契約期限日取得 = CDate(cellValue)
End Function

' 借入限度額取得関数
' C24セルから借入限度額を取得する
Public Function 借入限度額取得(toolSheet As Worksheet) As Currency
    Dim cellValue As Variant

    ' セル値を取得
    cellValue = toolSheet.Cells(借入限度額行, 借入限度額列).Value

    ' 空白チェック
    If cellValue = "" Or IsEmpty(cellValue) Then
        Err.Raise 13, "借入限度額取得", "借入限度額が設定されていません。C24セルに借入限度額を入力してください。"
    End If

    ' 数値型チェック
    If Not IsNumeric(cellValue) Then
        Err.Raise 13, "借入限度額取得", "借入限度額が数値型ではありません。C24セルに正しい金額を入力してください。"
    End If

    ' 負の値チェック
    If CDbl(cellValue) < 0 Then
        Err.Raise 13, "借入限度額取得", "借入限度額は0以上の値を入力してください。"
    End If

    ' Currency型に変換して返す
    借入限度額取得 = CCur(cellValue)
End Function

' 借入利率取得関数
' 指定されたシートの29行目からB列が空白になるまで、B列（借入利率）とC列（開始日）のデータを取得
Public Function 借入利率取得(toolSheet As Worksheet) As Variant
    Dim 現在行 As Long
    Dim 結果() As Variant
    Dim 行数 As Long
    Dim i As Long
    ...
    Do While toolSheet.Cells(現在行, 借入利率列).Value <> "" And 行数 < 借入利率最大行数
        行数 = 行数 + 1
        現在行 = 現在行 + 1
    Loop

    ' 途中行が空白で後続行にデータがある場合のチェック
    Dim チェック行 As Long
    For チェック行 = 現在行 To 借入利率開始行 + 借入利率最大行数 - 1
        If toolSheet.Cells(チェック行, 借入利率列).Value <> "" Then
            If チェック行 = 現在行 Then
                Err.Raise 13, "借入利率取得", "借入利率が設定されていません。"
            Else
                Err.Raise 13, "借入利率取得", "借入利率データに空白行があります。" & (チェック行 - 借入利率開始行 + 1) & "行目にデータが設定されていますが、途中に空白行が存在します。"
            End If
        End If
    Next チェック行
    ...
    ' データが存在しない場合はエラーを発生させる
    If 行数 = 0 Then
        Err.Raise 1000, "借入利率取得", "借入利率データが設定されていません。"
    End If
    ...
    For i = 1 To 行数
        ...
        ' B列（借入利率）を取得
        利率値 = toolSheet.Cells(現在行, 借入利率列).Value
        ...
        ' C列（開始日）を取得
        開始日値 = toolSheet.Cells(現在行, 借入利率開始日列).Value
...
Public Function 遅延損害金利率取得(toolSheet As Worksheet) As Variant
    Dim 現在行 As Long
    Dim 結果() As Variant
    Dim 行数 As Long
    Dim i As Long
    ...
    Do While toolSheet.Cells(現在行, 遅延損害金利率列).Value <> "" And 行数 < 遅延損害金利率最大行数
        行数 = 行数 + 1
        現在行 = 現在行 + 1
    Loop
    ...
    ' 途中行が空白で後続行にデータがある場合のチェック
    Dim チェック行 As Long
    For チェック行 = 現在行 To 遅延損害金利率開始行 + 遅延損害金利率最大行数 - 1
        If toolSheet.Cells(チェック行, 遅延損害金利率列).Value <> "" Then
            If チェック行 = 現在行 Then
                Err.Raise 13, "遅延損害金利率取得", "遅延損害金利率が設定されていません。"
            Else
                Err.Raise 13, "遅延損害金利率取得", "遅延損害金利率データに空白行があります。" & (チェック行 - 遅延損害金利率開始行 + 1) & "行目にデータが設定されていますが、途中に空白行が存在します。"
            End If
        End If
    Next チェック行
    ...
    ' データが存在しない場合はエラーを発生させる
    If 行数 = 0 Then
        Err.Raise 13, "遅延損害金利率取得", "遅延損害金利率データが設定されていません。最低1件のデータが必要です。"
    End If
    ...
    For i = 1 To 行数
        ...
        ' B列（遅延損害金利率）を取得
        利率値 = toolSheet.Cells(現在行, 遅延損害金利率列).Value
        If Not IsNumeric(利率値) Then
        ...
        ' C列（開始日）を取得
        開始日値 = toolSheet.Cells(現在行, 遅延損害金利率開始日列).Value
        If 開始日値 = "" Or IsEmpty(開始日値) Then
            ' 最初のレコードで空白の場合は最小日付をセット
            If i = 1 Then
                結果(i, 2) = 日付初期値
        ...
        Else
            ...
            ' 最初のレコードではなければ、前レコードの日付より小さいか同じ場合エラーとする
            If i > 1 Then
                If 結果(i, 2) <= 結果(i - 1, 2) Then
                    Err.Raise 13, "遅延損害金利率取得", "開始日が前のレコードより小さいか同じです。行: " & 現在行 & "（前レコード: " & 結果(i - 1, 2) & "、現在: " & 結果(i, 2) & "）"
                End If
            End If
        End If
...
Public Function 返済履歴情報取得(toolSheet As Worksheet) As Variant
    Dim 現在行 As Long
    Dim 結果() As Variant
    Dim 行数 As Long
    Dim i As Long
    ...
    Do While toolSheet.Cells(現在行, 返済履歴日付列).Value <> "" And 行数 < 返済履歴情報最大行数
        行数 = 行数 + 1
        現在行 = 現在行 + 1
    Loop
    ...
    For i = 1 To 行数
        ' B列（日付）を取得
        Dim 日付値 As Variant
        日付値 = toolSheet.Cells(現在行, 返済履歴日付列).Value
        If Not IsDate(日付値) Then
            Err.Raise 13, "返済履歴情報取得", 現在行 & "行目のB列（日付）が日付ではありません。"
        End If
        結果(i, 1) = CDate(日付値)

        ' 最初のレコードではなければ、前レコードの日付より小さい場合エラーとする
        If i > 1 Then
            If 結果(i, 1) < 結果(i - 1, 1) Then
                Err.Raise 13, "返済履歴情報取得", "日付が前のレコードより小さいです。行: " & 現在行 & "（前レコード: " & 結果(i - 1, 1) & "、現在: " & 結果(i, 1) & "）"
            End If
        End If

        ' C列（摘要）を取得
        Dim 摘要値 As String
        摘要値 = CStr(toolSheet.Cells(現在行, 返済履歴摘要列).Value)
        If Trim(摘要値) = "" Then
            Err.Raise 13, "返済履歴情報取得", 現在行 & "行目のC列（摘要）が空白です。"
        End If
        結果(i, 2) = 摘要値

        ' D列（出金金額）を取得
        Dim 出金金額値 As Variant
        出金金額値 = toolSheet.Cells(現在行, 返済履歴出金金額列).Value
        If Trim(CStr(出金金額値)) = "" Then
            Err.Raise 13, "返済履歴情報取得", 現在行 & "行目のD列（出金金額）が空白です。"
        End If
    ...
    ' 途中行が空白で後続行にデータがある場合のチェック
    Dim チェック行 As Long
    For チェック行 = 現在行 To 返済履歴開始行 + 返済履歴情報最大行数 - 1
        If toolSheet.Cells(チェック行, 返済履歴日付列).Value <> "" Or _
           toolSheet.Cells(チェック行, 返済履歴摘要列).Value <> "" Or _
           toolSheet.Cells(チェック行, 返済履歴出金金額列).Value <> "" Then
            If チェック行 = 現在行 Then
                Err.Raise 13, "返済履歴情報取得", "返済履歴情報が設定されていません。"
            Else
                Err.Raise 13, "返済履歴情報取得", "返済履歴情報データに空白行があります。" & (チェック行 - 返済履歴開始行 + 1) & "行目にデータが設定されていますが、途中に空白行が存在します。"
            End If
        End If
    Next チェック行
...
' 計算期間の最初日を計算する関数
Public Function 計算期間最初日取得(toolSheet As Worksheet) As Date
    Dim 返済予定データ As Variant
    Dim 最初日 As Date
    Dim 入出金データ As Variant
    Dim i As Long
